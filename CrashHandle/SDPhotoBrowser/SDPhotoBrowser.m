...
- (void)deleteImage
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:deleteImageForindex:)]) {
        CGFloat x = _scrollView.contentOffset.x;
        CGFloat w = _scrollView.bounds.size.width;
        CGSize size = _scrollView.contentSize;
        int index = x / w;
        NSInteger newIndex = [self.delegate photoBrowser:self deleteImageForindex:index];
        
        //SDBrowserImageView *imageView = _scrollView.subviews[index];
       // [imageView removeFromSuperview];
        
        //[self setupImageOfImageViewForIndex:newIndex];
        if (newIndex == 0) {
            [self photoClick:nil];
        }else if(newIndex > index){
            if (index != 0) {
                SDBrowserImageView *imageView1 = _scrollView.subviews[index-1];
                [_scrollView insertSubview:imageView1 atIndex:index];
            }
            _scrollView.contentOffset = CGPointMake(x+w, 0);
        }else if (newIndex < index){
            _scrollView.contentOffset = CGPointMake(x-w, 0);
        }
       // _scrollView.contentSize = CGSizeMake(size.width - w, size.height);
    }
}
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    _willDisappear = YES;
    //由于直接从索引取subview会出现混乱所以使用tag，但是需要与外部一一对应：例如都加100；
    SDBrowserImageView *currentImageView = (SDBrowserImageView *)recognizer.view;
    NSInteger currentIndex = currentImageView.tag + 100;
    UIView *sourceView = [self.sourceImagesContainerView viewWithTag:currentIndex];
...
- (void)layoutSubviews
{
    ...
    _deleteButton.frame = CGRectMake(self.bounds.size.width-80, self.bounds.size.height - 70, 50, 25);
}
...
- (void)showFirstImage
{
    ...
    UIView *sourceView = [self.sourceImagesContainerView viewWithTag:self.currentImageIndex+100];
